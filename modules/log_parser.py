...
class LogParser():
            ...
            if "CActor::Kill" in line and not self.check_ignored_victims(line) and upload_kills:
                ...
                # Log a message for the current user's death
                    ...
                    if kill_result["result"] == "killed":
                        ...
                        weapon_raw = kill_result["data"].get("weapon_raw")
                        death_cause = self._classify_player_death(killer_name, weapon_raw, weapon_name)

                        if death_cause == "player":
                            ...
                            if hasattr(self.gui, "record_pvp_event"):
                                self.gui.record_pvp_event(self.game_mode, "death", killer_name)
                ...
                # Log a message for the current user's kill
                    ...
                    if hasattr(self.gui, "record_pvp_event"):
                        self.gui.record_pvp_event(self.game_mode, "kill", kill_result['data']['victim'])
    ...
    def _classify_player_death(self, killer_name, weapon_raw, weapon_name):
        killer_lower = (killer_name or "").strip().lower()
        weapon_raw_lower = (weapon_raw or "").lower()
        weapon_name_lower = (weapon_name or "").lower()

        collision_markers = ("collision", "crash", "impact")
        if any(marker in weapon_raw_lower for marker in collision_markers) or any(
            marker in weapon_name_lower for marker in collision_markers
        ):
            return "collision"

        if "selfdestruct" in weapon_raw_lower or "self_destruct" in weapon_raw_lower:
            return "environment"

        if not killer_lower:
            return "environment"

        current_handle = (self.rsi_handle.get("current") or "").strip().lower()
        if current_handle and killer_lower == current_handle:
            return "environment"

        if "::" in killer_lower:
            return "environment"

        if killer_lower in {"unknown", "environment", "suicide"}:
            return "environment"
        ...
        if any(indicator in killer_lower for indicator in environment_indicators):
            return "environment"
